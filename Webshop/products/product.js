...
function filterProducten(genre) {
  const producten = document.querySelectorAll(".product");
  const selectedGenre = genre.toLowerCase();

  producten.forEach(function (product) {
    const genreElement = product.querySelector(".genre");
    const productGenres = genreElement
      ? genreElement.innerText.toLowerCase()
      : "";

    if (selectedGenre === "all" || productGenres.includes(selectedGenre)) {
